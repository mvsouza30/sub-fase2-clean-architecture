...
public class CarDTOMapper {
    CreateCarResponse toResponse(Car car) {
        return new CreateCarResponse(car.marca(), car.modelo(), car.cor(), car.ano(), car.preco());
    }

    public Car toCar(CreateCarRequest request) {
        return new Car(request.marca(), request.modelo(), request.cor(), request.ano(), request.preco());
    }
}
